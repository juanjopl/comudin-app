...
try {
    ...
    // Actualiza el contador de "me gusta" del post según el estado de la solicitud
    if($status=="heart") {
        $sql = "UPDATE instaposts SET likes = likes + 1 WHERE idinstapost = :idPost;";
    } else {
        $sql = "UPDATE instaposts SET likes = likes - 1 WHERE idinstapost = :idPost;";
    }
    ...
    if($res) {
        // Si el estado es "corazón", inserta un nuevo registro en la tabla de "me gusta"
        // Si no, elimina el registro correspondiente de la tabla de "me gusta"
        if($status=="heart") {
            $sql = "INSERT INTO instalikes (post, idUser) VALUES (:post, :idUser);";
        } else {
            $sql = "DELETE FROM instalikes WHERE post = :post AND idUser = :idUser;";
        }
        $statement = $con->prepare($sql);
